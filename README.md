...
```mermaid
flowchart LR
    subgraph MAC["macOS · Aishow.app (Swift, ~4k lines) — eyes, ears, hands"]
        HK["⌘ long-press<br/>push-to-talk"] --> SCAN["scan<br/>frontmost app · URL · focused field"]
        SCAN --> DETECT["detect()<br/>website_form · linkedin_dm · casual_en · email_en · translate"]
        HK --> REC["chant<br/>record voice"]
        APPROVE["Approval pane<br/>sources · editable text · target app"] -- approve --> PASTE["cast<br/>Cmd+V at cursor · clipboard restored · never Enter"]
    end

    REC --> STT["OpenAI<br/>gpt-4o-transcribe"]

    subgraph TF["TrueForge — local agent harness (:8790) — the brain"]
        AGENT["Saved agent 'aishow'<br/>instructions = harness/spells/*.md"] --> SESSION["one session per company domain"]
        AGENT --> MODEL["OpenAI<br/>gpt-5.2"]
        AGENT --> MCP["remote MCP connector"]
    end

    DETECT --> AGENT
    STT --> AGENT
    MCP <--> BD["Bright Data Web MCP<br/>search_engine · scrape_as_markdown<br/>home · /about · /products · news"]
    SESSION -- "SSE → {sources, text, note}" --> APPROVE
    APPROVE -. "reject with reason" .-> SESSION

    QODO["Qodo<br/>/agentic_review on every PR<br/>rules in .pr_agent.toml"] -.-> MAC
```
